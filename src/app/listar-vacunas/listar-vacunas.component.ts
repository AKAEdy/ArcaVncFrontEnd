...
export class ListarVacunasComponent implements OnInit {
    ...
    if(this.vacunasid.nombre === undefined || this.vacunasid.tipo === undefined || this.vacunasid.descripcion === undefined
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.vacunaService.updateUsingPUT5(this.vacunasid, this.vacunasid.id).subscribe(data =>{
            Swal.fire({
              position: 'center',
              icon: 'success',
  ...
  getVacunasById(id: number) {
    this.vacunaService.getByIdUsingGET5(id).subscribe(data => {
      this.vacunasid = data
      this.mostrarEditar()
      console.log(id);
  ...
  deleteVacunas(id: number) {
    ...
    this.vacunaService.deleteUsingDELETE5(id).subscribe(data => {
